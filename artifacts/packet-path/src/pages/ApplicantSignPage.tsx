    ...
    <div className="min-h-screen luminous-gradient flex items-center justify-center p-6">
      <div className="tahoe-panel max-w-lg rounded-[34px] p-8 text-center">
        <div className="mx-auto mb-5 flex h-16 w-16 items-center justify-center rounded-full border border-white/25 bg-white/10">
          <Icon className={state === "success" ? "text-lime-200" : "text-amber-200"} size={32} />
        </div>
...
function SignatureCanvas({ onChange }: { onChange: (dataUrl: string) => void }) {
  ...
  return (
    <div className="relative rounded-[24px] border border-white/30 bg-white/90 p-2">
      ...
      />
      <div className="pointer-events-none absolute inset-0 flex items-center justify-center text-lime-950/20">Sign here</div>
      <button type="button" onClick={clear} className="absolute right-4 top-4 rounded-full bg-white p-2 text-lime-950 shadow">
        <RotateCcw size={15} />
      </button>
    </div>
  );
      ...
      <main className="tahoe-workspace mx-auto max-w-7xl">
        <header className="mb-8 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div className="flex items-center gap-3">
            <div className="flex h-12 w-12 items-center justify-center rounded-2xl border border-lime-200/30 bg-lime-300/10 shadow-[0_0_30px_rgba(183,236,72,.25)]">
              <PenTool className="text-lime-200" />
            </div>
            <div>
              <div className="text-[10px] font-semibold uppercase tracking-[0.35em] text-lime-200/70">Occu-Med</div>
        ...
        <section className="mb-8 grid gap-6 lg:grid-cols-[1fr_340px]">
          <div>
            <div className="text-[12px] font-semibold uppercase tracking-[0.35em] text-lime-200/75">Action Required</div>
          ...
          <div className="tahoe-panel rounded-[28px] p-5">
            <div className="text-[11px] uppercase tracking-[0.25em] text-lime-200/75">Recipient</div>
        ...
        <div className="mb-6 grid grid-cols-2 gap-3">
          <div className={step === "review" ? "rounded-2xl border border-lime-200 bg-lime-300 px-4 py-3 text-center text-sm font-semibold text-lime-950" : "rounded-2xl border border-white/15 bg-white/10 px-4 py-3 text-center text-sm font-semibold text-white/55"}>1. Review</div>
          <div className={step === "sign" ? "rounded-2xl border border-lime-200 bg-lime-300 px-4 py-3 text-center text-sm font-semibold text-lime-950" : "rounded-2xl border border-white/15 bg-white/10 px-4 py-3 text-center text-sm font-semibold text-white/55"}>2. Sign</div>
        </div>
        ...
        {step === "review" ? (
          <section className="grid gap-6 lg:grid-cols-[1fr_260px]">
            ...
            <aside className="tahoe-panel flex flex-col justify-between rounded-[30px] p-5">
              <div>
                <div className="text-[11px] uppercase tracking-[0.25em] text-lime-200/75">Step 1</div>
          ...
          <section className="mx-auto max-w-3xl">
            <div className="tahoe-panel rounded-[34px] p-6">
              <h2 className="flex items-center gap-3 text-3xl font-semibold text-white"><PenTool className="text-lime-200" /> Apply your signature</h2>
              <div className="mt-5 inline-flex rounded-2xl border border-white/20 bg-white/10 p-1">
                <button onClick={() => setMode("typed")} className={mode === "typed" ? "rounded-xl bg-lime-300 px-4 py-2 text-sm font-semibold text-lime-950" : "rounded-xl px-4 py-2 text-sm font-semibold text-white/60"}>Type</button>
                <button onClick={() => setMode("drawn")} className={mode === "drawn" ? "rounded-xl bg-lime-300 px-4 py-2 text-sm font-semibold text-lime-950" : "rounded-xl px-4 py-2 text-sm font-semibold text-white/60"}>Draw</button>
              </div>
              <div className="mt-5">
                {mode === "typed" ? (
                  <input value={typedName} onChange={e => setTypedName(e.target.value)} placeholder="Type your full legal name" className="w-full rounded-[24px] border border-white/25 bg-white/85 px-5 py-4 text-3xl text-lime-950 outline-none" style={{ fontFamily: "Georgia, serif" }} />
        ...
        <footer className="mt-8 grid gap-4 md:grid-cols-3">
          <div className="tahoe-panel rounded-[24px] p-4 text-sm text-white/65"><Shield className="mb-2 text-lime-200" /> HIPAA audit trail recorded.</div>
          <div className="tahoe-panel rounded-[24px] p-4 text-sm text-white/65"><Lock className="mb-2 text-lime-200" /> Secure token link. No portal account required.</div>
          <div className="tahoe-panel rounded-[24px] p-4 text-sm text-white/65"><CheckCircle className="mb-2 text-lime-200" /> Signed PDF generated after completion.</div>
        </footer>
      </main>
    </div>
  );
